...
import pdfIcon from '../assetGuru/pdfIcon.svg';
...
const Materi: Component = () => {
  ...
  // State baru untuk menyimpan keterangan tiap file
  const [fileDescriptions, setFileDescriptions] = createSignal<string[]>([]);
  ...
  // State untuk menyimpan materi yang sudah ditambahkan
  const [addedMateri, setAddedMateri] = createSignal({
    judul: '',
    penjelasan: '',
    files: [] as File[],
    fileDescriptions: [] as string[],
    lampiran: [] as File[],
  ...
  // Handler untuk menangani perubahan pada input keterangan
  const handleKeteranganChange = (event: Event) => {
    const target = event.target as HTMLInputElement;
    ...
    if (index !== null) {
      ...
      setFileDescriptions(updatedDescriptions); // Update deskripsi sesuai file yang dipilih
    }
  };
  ...
  const handleFileMateriChange = (event: Event) => {
    const target = event.target as HTMLInputElement;
    if (target.files && target.files.length > 0) {
      const file = target.files[0];

      if (allowedFormats.includes(file.type)) {
        if (selectedFiles().length < 3) {  // Pastikan file tidak lebih dari 3
          setSelectedFiles([...selectedFiles(), file]);
          setFileDescriptions([...fileDescriptions(), '']);
          setNotification(null); // Clear notification on successful upload
        }
      } else {
        setNotification('Format file tidak didukung');
      }
    }
  };
  ...
  const handleHapusFile = (index: number) => {
    ...
    const updatedDescriptions = fileDescriptions().filter((_, i) => i !== index);
  ...
  return (
    <div class="contentMateri">
      <div class="contentMateri1">
        <div>
          <h1 class="judul">Materi Ajar - Ilmu Pengetahuan Alam</h1>
          <div class="pengampu">
        ...
        <div class={`uploadBackground ${previewUrl() ? 'no-border' : ''}`} style={{ "background-image": previewUrl() ? `url(${previewUrl()})` : 'none' }}>
        {!previewUrl() && (
            <>
      ...
      {isContentKosong() ? (
          ...
          <div class="bungkusContentIsi">
            <div class="contentIsi">
              ...
              <div class="fileList">
                {addedMateri().files.map((file, index) => (
                  <div class="fileItem">
                    {file.type.includes('image') ? (
                      <img src={URL.createObjectURL(file)} alt={`File ${index}`} class="uploadedImage" />
                    ) : (
                      <video src={URL.createObjectURL(file)} controls class="uploadedVideo"></video>
                    )}
                    <p class="fileDescription">{addedMateri().fileDescriptions[index]}</p>
                  </div>
                ))}
              </div>
      ...
      {isTambahMateriPopupVisible() && (
        <div class="popupOverlay" id="tambah-materi-popup">
          ...
          <div class="popupHeaderMateri">
              ...
              <h2 class="titleHeaderMateri">Tambah Materi</h2>
            ...
            <div class="popupContentMateri">
                  <div class="gabunganLabel">
                    <label class="labelInput">Judul Bab / Materi</label>
                  ...
                  <div class="gabunganLabel2">
                    <label class="labelInput">Deskripsi Materi</label>
                    <label class="bintangMerah">*</label>
                  </div>
                  ...
                  <div>
                    <div class="bungkusText1">
                      <label class="textMateri1">Foto / Video</label>
                      <label class="textMateri2">Keterangan</label>
                    </div>
                    <div class="bungkusInput1">
                    {selectedFiles().map((file, index) => (
                        <div
                        class="file-box"
                        onMouseEnter={() => setHoveredFileIndex(index)}  // Set index pada hover
                        onMouseLeave={() => setHoveredFileIndex(null)}   // Reset index saat tidak hover
                        onClick={() => handleHapusFile(index)}
                      >
                        {/* Tampilkan 'Hapus' jika di-hover, jika tidak tampilkan nama file (IMG atau VID) */}
                        {hoveredFileIndex() === index 
                          ? <img src={hapusIcon} alt="Hapus Icon" class="hapusIcon"/>  
                          : file.type.includes('image') ? `IMG${index + 1}` : `VID${index + 1}`}
                      </div>
                    ))}

                    {/* Tombol "Tambah" jika file belum mencapai 3 */}
                    {selectedFiles().length > 0 && selectedFiles().length < 3 && (
                      <button class="tambahFileButton" onClick={handleTelusuriMateriClick}>
                        <img src={tambahIcon} alt="Tambah Icon" class="tambahFileIcon"/>  {/* Optional: icon tambah */}
                      </button>
                    )}

                      {/* Upload button jika belum ada file */}
                      {selectedFiles().length === 0 && (
                        <button class="uploadFile1" onClick={handleTelusuriMateriClick}>
                      ...
                      <input
                        ...
                        class="inputKeterangan1"
                        value={activeFileIndex() !== null ? fileDescriptions()[activeFileIndex()] : ""}
                        onInput={handleKeteranganChange}
                      />
                      ...
                      {selectedFiles().length > 0 && (
                        <select
                          class="fileComboBox"
                          onChange={(e) => handleFileSelect(parseInt(e.currentTarget.value))}
                        >
                          {selectedFiles().map((file, index) => (
                            <option value={index}>
                              {file.type.includes('image') ? `IMG${index + 1}` : `VID${index + 1}`}
      ...
      {isHapusPopupVisible() && (
        <div class="popupOverlay">
          <div class="popupHapusMateri">
            <p class="textKonfirmasi">Apakah Anda yakin ingin menghapus materi ini?</p>
